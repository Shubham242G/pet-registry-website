...
function getSaleEndDate(): string {
...
function applySaleDiscount(price: number): number {
  if (!isSaleActive()) return price;
  return price * (1 - SALE_DISCOUNT);
}

function getOriginalPrice(city: string): number {
  const cityLower = city?.toLowerCase() || '';
  if (!VALID_CITIES.includes(cityLower)) return 0;
...
// ✅ STRICT PRICE FUNCTION with SALE
function getPrice(city: string, tagOption: string) {
  const cityLower = city?.toLowerCase() || '';
  ...
  if (['ghaziabad', 'gurgaon'].includes(cityLower)) {
    ...
    basePrice = 799;
  } else if (['jaipur'].includes(cityLower)) {
    basePrice = 1799;
  }
  ...
  const gstRate = 0.18;
  const gstAmount = basePrice * gstRate;
  let total = basePrice + gstAmount;
  
  // ✅ Apply 25% Independence Day Sale
  const saleActive = isSaleActive();
  let originalTotal = total;
  let discountAmount = 0;
  
  if (saleActive) {
    discountAmount = total * SALE_DISCOUNT;
...
export default function AddPetModal({ 
  ...
  try {
    ...
    petPrice = { basePrice: 0, gstAmount: 0, gstRate: 18, total: 0, tagDeliveryCost: 0, saleActive: false, discountPercent: 0, discountAmount: 0, originalTotal: 0 };
    priceError = error.message;
  }
              ...
              <div style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ color: "rgba(255,255,255,0.85)", fontSize: 11 }}>
                    Ends {getSaleEndDate()} · {getDaysRemaining()} days left
                  </div>
                </div>
                <div style={{
          ...
          <div style={{ overflowY: "auto", flex: 1, padding: "16px 20px 20px" }}>
            ...
            {!success && step === 2 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                {/* ✅ SALE BANNER in payment step */}
                {petPrice.saleActive && (
                  <div style={{
                    background: "linear-gradient(135deg, #2C1A0E 0%, #1F1108 100%)",
                    borderRadius: 11,
                    padding: "14px 16px",
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ color: "white", fontSize: 14, fontWeight: 700 }}>
                        Independence Day Sale — {petPrice.discountPercent}% OFF!
                      </div>
                      <div style={{ color: "rgba(255,255,255,0.6)", fontSize: 12 }}>
                        You save ₹{petPrice.discountAmount.toFixed(2)} · Ends {getSaleEndDate()}
                      </div>
                    </div>
                    <div style={{
                      background: "#FF8C3A",
                ...
                {priceError ? (
                  ...
                  <div style={{ background: "#FFFCF8", borderRadius: 11, padding: "14px 16px", outline: "1px solid rgba(44,26,14,0.10)", outlineOffset: -1 }}>
                    ...
                    {/* ✅ Show original price with strike-through if sale is active */}
                    {petPrice.saleActive && (
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                        <span style={{ color: "#7A5C40", fontSize: 12 }}>Original Price</span>
                        <span style={{ color: "#A68660", fontSize: 12, textDecoration: "line-through" }}>
                          ₹{petPrice.originalTotal.toFixed(2)}
                        </span>
                      </div>
                    )}
                    ...
                    {/* ✅ Show discount if sale is active */}
                    {petPrice.saleActive && (
                      <div style={{ 
                        display: "flex", 
                        ...
                        <span style={{ color: "#C04E06", fontSize: 12, fontWeight: 600 }}>
                          🎉 Independence Day Discount
                        </span>
                        <span style={{ color: "#1A6B3A", fontSize: 12, fontWeight: 700 }}>
                          -₹{petPrice.discountAmount.toFixed(2)}
                        </span>
                      </div>
                    )}
                    ...
                    {petPrice.saleActive && (
                      <div style={{ 
                        ...
                        <span style={{ color: "#1A6B3A", fontSize: 12, fontWeight: 600 }}>
                          🇮🇳 You save ₹{petPrice.discountAmount.toFixed(2)} on this registration!
                        </span>
                      </div>
                    )}
                  </div>
