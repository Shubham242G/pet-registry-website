...
export default function LoginModal({ isOpen, onClose, onSwitchToRegister }: LoginModalProps) {
  // Removed: login (email), register — not needed anymore
  const { sendWhatsAppOTP, verifyWhatsAppOTP, completeWhatsAppRegistration } = useAuth();
  const router = useRouter();

  // Removed: authMethod state (was "email" | "whatsapp") — always WhatsApp now
  const [step, setStep] = useState<"phone" | "otp" | "register">("phone");

  // Removed: email, password state

  // WhatsApp login state — all kept exactly as before
  ...
  // Removed: handleEmailLogin — not needed

  // Send WhatsApp OTP — unchanged
      ...
      <div style={{ width: 420, maxWidth: '100%', background: '#FFFCF8', borderRadius: 18, boxShadow: '0px 24px 80px rgba(44,26,14,0.18)', outline: '1px solid rgba(44,26,14,0.10)', outlineOffset: -1, position: 'relative' }}>
        ...
        <div style={{ padding: 28 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 9.5, fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '1.33px', color: '#E8600A' }}>Quick sign in</span>
          </div>

          <h2 style={{ fontSize: 26, fontWeight: 900, lineHeight: '28.6px', marginBottom: 4 }}>
            <span style={{ color: '#2C1A0E' }}>Login with </span>
